...
class FileGenerateTest(unittest.TestCase):
    ...
    def test_file_generate(self):
        ...
        ds = Dataset('/Users/user/Desktop/netcdf/aiw_task19_cin_input_20220207.nc', 'w', format="NETCDF4")  # 새 nc파일 생성
        ...
        values = np.array(data600['rh_600'])[1100:1500, 700:1100] #[0]
        ds.createVariable('INPUTDATA/' + 'rh_600' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
        ds['INPUTDATA/' + 'rh_600' + '/h000'][:, :] = values

        values = np.array(data700['rh_700'])[1100:1500, 700:1100]
        ds.createVariable('INPUTDATA/' + 'rh_700' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
        ds['INPUTDATA/' + 'rh_700' + '/h000'][:, :] = values

        values = np.array(data850['rh_850'])[1100:1500, 700:1100]
        ds.createVariable('INPUTDATA/' + 'rh_850' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
        ds['INPUTDATA/' + 'rh_850' + '/h000'][:, :] = values

        values = np.array(data925['rh_925'])[1100:1500, 700:1100]
        ds.createVariable('INPUTDATA/' + 'rh_925' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
        ds['INPUTDATA/' + 'rh_925' + '/h000'][:, :] = values

        values = np.array(data950['rh_950'])[1100:1500, 700:1100]
        ds.createVariable('INPUTDATA/' + 'rh_950' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
        ds['INPUTDATA/' + 'rh_950' + '/h000'][:, :] = values

        values = np.array(data1000['rh_1000'])[1100:1500, 700:1100]
        ds.createVariable('INPUTDATA/' + 'rh_1000' + '/h000', float, ('latitude', 'longitude'))  # 변수공간 생성
